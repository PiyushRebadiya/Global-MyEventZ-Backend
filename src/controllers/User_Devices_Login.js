...
const AddUserDevicesLogin = async(req, res)=>{
    const { DeviceName = '' , DeviceType = '' , DeviceOS = '', BrowserName = '', Token, UserUkeyId = '', OrganizerUkeyId = '', NotificationToken = '' } = req.body;
    ...
    try{
        ...
        const insertQuery = `
            INSERT INTO user_devices (DeviceName, DeviceType, DeviceOS, BrowserName, Token, NotificationToken, UserUkeyId, OrganizerUkeyId,  Log_In, Log_In_Time, Log_Out, IpAddress, HostName, EntryDate) VALUES (${setSQLStringValue(DeviceName)}, ${setSQLStringValue(DeviceType)}, ${setSQLStringValue(DeviceOS)}, ${setSQLStringValue(BrowserName)}, ${setSQLStringValue(Token)}, ${setSQLStringValue(NotificationToken)}, ${setSQLStringValue(UserUkeyId)}, ${setSQLStringValue(OrganizerUkeyId)}, 1, GETDATE(), 0, ${setSQLStringValue(IPAddress)}, ${setSQLStringValue(ServerName)}, ${setSQLStringValue(EntryTime)});
        `
